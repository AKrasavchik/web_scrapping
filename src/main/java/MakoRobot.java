              ...
              //  String linkToScraping = preLink + link;
                if (i != allLinks.size() - 1) {
                    /*if (!link.equals(allLinks.get(i + 1).attr("href"))
                            && scanLink(link) == Definitions.MAKO_PAGE && (link.contains("/news/")) ) {*/
                       if(!link.equals(allLinks.get(i + 1).attr("href")) && link.equals("news")){
    ...
    public int countInArticlesTitles(String text) {
        ...
        try {
            ...
            for (int i = 0; i < allLinks.size(); i++){
               ...
               // String linkToScraping = preLink + link;
                if (i != allLinks.size() - 1) {
                    if (!link.equals(allLinks.get(i + 1).attr("href")) && (link.contains("news"))){
                        Document makoPage = Jsoup.connect(link).get();
                        Elements mainTitle = makoPage.getElementsByTag("h1");
                        Elements subTitle = makoPage.getElementsByTag("h2");
